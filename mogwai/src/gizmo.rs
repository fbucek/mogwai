...
pub struct Gizmo<T: JsCast> {
  pub(crate) element: Arc<JsValue>,
  pub(crate) phantom: PhantomData<T>,
  pub(crate) callbacks: HashMap<String, Arc<Closure<dyn FnMut(JsValue)>>>,
  pub(crate) window_callbacks:
    HashMap<String, Arc<Closure<dyn FnMut(JsValue)>>>,
...
impl<T: JsCast + AsRef<Element>> Gizmo<T> {
  ...
  /// TODO: Gizmo::attribute should take value: impl Display
...
//  pub fn gizmos<X: AsRef<Node>>(
//    &mut self,
//    init: Vec<Gizmo<X>>,
//    rx: Receiver<Vec<GizmoBuilder<Node>>>,
//  ) {
//    // Save a clone so we can drop_responder if this gizmo goes out of scope
...
//    let node: &Node = self.as_ref();
//    prev_gizmos.iter().for_each(|gizmo: &Gizmo| {
//      node
//        .append_child(gizmo.element_ref())
//        .expect("Could not add initial child gizmo");
//    });
//    rx.respond(move |gizmo_builders: &Vec<GizmoBuilder<Node>>| {
...
//        .collect();
//
//      let max_gizmos_len = usize::max(gizmos.len(), prev_gizmos.len());
//
//      for i in 0..max_gizmos_len {
//        let previous = prev_gizmos.get(i);
//        let new = gizmos.get(i);
//
...
//            node
//              .replace_child(
//                new.element.dyn_ref().unwrap_throw(),
//                prev.element.dyn_ref().unwrap_throw(),
//              )
//              .unwrap_throw();
//          } else {
//            node
//              .remove_child(prev.element.dyn_ref().unwrap_throw())
//              .unwrap_throw();
//          }
//        } else {
//          let new_node = new.unwrap_throw().element.dyn_ref().unwrap_throw();
//          node.append_child(new_node).unwrap_throw();
//        }
//      }
//
//      prev_gizmos = gizmos;
...
impl<T: JsCast> Drop for Gizmo<T> {
  fn drop(&mut self) {
    let count = Arc::strong_count(&self.element);
    if count <= 1 {
      self.element.dyn_ref::<Node>().map(|node| {
        node.parent_node().iter().for_each(|parent| {
